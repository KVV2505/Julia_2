#=Определить тип, позволяющий итерировать все размещения без повторений элементов заданного n элементного множества (в частности, последовательность {1,2,...,n} может задаваться просто числом n) по k.=#
